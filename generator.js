var mode = "interface"; // = "module"
// parser for https://www.bitcoin.de/de/api/tapi/v1/docu

var myMappings = {
   "Sonstiges.showOrderbookCompact.Response.Success_200.orders":
      "Sonstiges.showOrderbookCompact.Response.Orders",
   "Sonstiges.showOrderbookCompact.Response.Orders.bids":
      "Sonstiges.showOrderbookCompact.Response.Bids[]",
   "Sonstiges.showOrderbookCompact.Response.Orders.asks":
      "Sonstiges.showOrderbookCompact.Response.Asks[]",
   "Orders.showOrderbook.Response.Success_200.orders":
      "Orders.showOrderbook.Response.Orders[]",
...
var myRefs = {
   // "bankenländer": ,
   table_showAccount_fidor_reservation:
      "#table_showAccountInfo_fidor_reservation",
   table_showAccountStatement_account_ledger:
      "#table_showAccountLedger_account_ledger",
...
function parseHeader(ele) {
   ...
   if (
      ele.previousSibling &&
      ele.previousSibling.tagName === "A" &&
      ele.previousSibling.name
   ) {
...
function makeType(x, { header, path }) {
   ...
   if (v) {
      if (v.type === "enum") return v.values.map(x => `"${x}"`).join(" | ");
   }
   ...
   if (a === "array") {
      ...
      if (myMappings[k]) {
         ...
         if (p.endsWith("[]"))
            return tsPath(path, p.substring(0, p.length - 2).split(".")) + "[]";
...
function makeField(x, iface) {
    ...
    */`);
   str.push(`
    ${x.Name.value.replace(/\*/g, "")}${x.Required &&
   x.Required.value &&
...
function categorize(eles) {
   ...
   for (const ele of eles) {
      ...
      for (const part of path) {
         let p = target.children.find(
            e => e.header === part && e.$type === "module"
         );
         if (!p) {
...
var { tree, allInterfaces } = categorize(
   $$("table")
      .slice(11)
      .map(tbl => {
...
var methods = $$("[class^='method_']").map(method => {
   ...
   let path = method.nextSibling.textContent;
   path = path.replace(/^https:\/\/api.bitcoin.de\/v1\//, "");
   let e = method;
   while(e && e.tagName !== 'H2') e = e.previousSibling;
